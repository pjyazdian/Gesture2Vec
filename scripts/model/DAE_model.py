"""This module contains models for Part a: Pose Representation Learning.

The 'DAE_Network' model is the default and is described in the paper.
All other models are provided only for experimental purposes and
are provided as-is.

Typical usage example:
    net = DAE_Network(135, 40)
...
class DAE_Network(nn.Module):
    ...
    def __init__(self, motion_dim: int, latent_dim: int):
        ...
        if latent_dim == -2:
            self.encoder = nn.Sequential(
                nn.Linear(motion_dim, 200),
            )
            self.decoder = nn.Sequential(
                nn.Linear(200, motion_dim),
            )
    ...
    def forward(
        self, x: torch.Tensor, get_latent: bool = False
    ) -> torch.Tensor | Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass through the net.

        If sentinel value of -1 was used during initialization then
        early termination of method occurs (See below for return values).

        Args:
            x: The training data as a PyTorch Tensor.
            get_latent: Boolean whether to return the latent code space.
...
class VQ_Frame(nn.Module):
    ...
    Attributes:
        dropout: A PyTorch Dropout layer with a 50% chance of dropout.
        encoder: A PyTorch Linear Layer.
        decoder: A PyTorch Linear layer.
        bachnorm:
        skip_vq:
        vq_components:
    ...
    def __init__(self, motion_dim: int, latent_dim: int, vae: bool, vq_components: int):
        ...
        if decay > 0.0:
            self.vq_layer = VQ_Payam_EMA(
                self.vq_components, latent_dim, self.commitment_cost, decay
            )
        else:
            self.vq_layer = VQ_Payam(
                self.vq_components, latent_dim, self.commitment_cost
            )

        # GS-Soft Vector Quantization
        self.gs_soft = False
        if self.gs_soft:
            self.vq_layer = VQ_Payam_GSOFT(
                self.vq_components, latent_dim, self.commitment_cost, decay
            )
...
class VQ_Payam(nn.Module):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, commitment_cost: float):
        """Initialize with the number and dimensions for the clusters.

        Args:
            num_embeddings: The integer number of clusters in the latent code space.
            embedding_dim: The integer dimension of the latent code space.
    ...
    def forward(
        ...
        Returns:
            A 4-Tuple:
            loss: A Tensor of loss calculations (MSELoss).
            quantized: A Tensor of the stop gradient.
            perplexity: A Tensor of perplexity values of the latent code space.
            encodings: A Tensor of the clusters in the latent code space.
        """
        # 1. Pre-Linear
...
class VAE_Network(nn.Module):
    ...
    Attributes:
        dropout: A PyTorch Dropout layer with a 50% chance of dropout.
        encoder: A PyTorch Linear Layer with a Tanh activation function.
        decoder: A PyTorch Linear layer.
        vae: A boolean indicating whether this is a VAE model.
        VAE_fc_mean:
        VAE_fc_std
        VAE_fc_decoder:
    """
