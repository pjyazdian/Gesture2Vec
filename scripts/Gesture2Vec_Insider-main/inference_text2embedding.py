...
from utils.data_utils import SubtitleWrapper as SubtitleWrapper_trinity, normalize_string as normalize_string_trinity

from utils.data_utils_twh import SubtitleWrapper as SubtitleWrapper_twh, normalize_string as normalize_string_twh
...
def plot_attentions(word_seq, best_guess, attention_list):
    ...
    # Set up axes
    ax.set_yticklabels([''] + word_seq.split(' ') +
                       ['<EOS>'], rotation=0)
...
def generate_gestures(args, pose_decoder, lang_model,
    ...
    for i in range(0, num_subdivision):
        ...
        for i_pre_seq in range(pose_decoder.n_pre_poses):
            pre_seq[0, i_pre_seq] = best_guess[0, len(best_guess[0])-pose_decoder.n_pre_poses+i_pre_seq]
...
../output/GENEA/VQ-VAE/VQVAE_checkpoint_3000.bin
    ...
    if DATASET_TYPE == 'trinity':
        ...
        for i in reversed(range(1, 3)):  # 11)):
            ...
            transcript_path = Path(pre_transcript_path + "val_2022_v1_" + str(i).zfill(3) + ".tsv")
