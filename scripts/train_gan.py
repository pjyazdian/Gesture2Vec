...
from model.text2embedding_model import text2embedding_model
from model.text2embedding_GAN_model import text2embedding_model_gan
from train_eval.train_seq2seq import train_iter_seq2seq, train_iter_c2g_seq2seq, train_iter_text2embedding, train_iter_text2embedding_GAN
...
def init_model(args, lang_model, pose_dim, _device):
    ...
    gen_disc = text2embedding_model_gan(args, pose_dim, n_frames, lang_model.n_words, args.wordembed_dim,
...
def train_epochs(args, train_data_loader, test_data_loader, lang_model, pose_dim, trial_id=None):
    ...
    # init model
    gen_disc, bce_loss = init_model(args, lang_model, pose_dim, device)

    # define optimizers
    gen_optimizer = optim.Adam(gen_disc.generator.parameters(), lr=args.learning_rate, betas=(0.5, 0.999))
    disc_optimizer = optim.Adam(gen_disc.discriminator.parameters(), lr=args.learning_rate, betas=(0.5, 0.999))
    # training
    global_iter = 0
    d_real_loss = []
    d_fake_loss = []
    for epoch in range(1, args.epochs+1):
        ...
        val_metrics = evaluate_testset(test_data_loader, gen_disc, torch.nn.MSELoss(), args)

        # save model
        if epoch % save_model_epoch_interval == 0 and epoch > 0:
            try:  # multi gpu
                gen_state_dict = gen_disc.module.state_dict()
            except AttributeError:  # single gpu
                gen_state_dict = gen_disc.state_dict()
        ...
        for iter_idx, data in enumerate(train_data_loader, 0):
            ...
            in_text, text_lengths, target_vec, in_audio, aux_info, sentence_leve_latents, c_portion = data
            ...
            # train
            loss = train_iter_text2embedding_GAN(args, epoch, in_text, text_lengths,
                                                 target_vec, c_portion,
                                                 gen_disc.generator, gen_disc.discriminator,
                                                 gen_optimizer, disc_optimizer)
            d_real_loss.append(loss[0])
            d_fake_loss.append(loss[1])
    ...
    plt.title("AAAAAAAAAAAAAAAAAAA")
    plt.show()
    losses = {'d_fake_loss': d_fake_loss,
              'd_real_loss': d_real_loss}
    pickle.dump(losses, open('losessss.plk', 'wb'))

def evaluate_testset(test_data_loader, gen_disc, loss_fn, args):
    # to evaluation mode
    gen_disc.train(False)
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            ...
            if debug:
                print(sentence_leve_latents.shape)
            ...
            out_latents = gen_disc.generator(in_text, text_lengths, target, None)
            if debug:
                print("Compare", out_latents.shape, target.shape)
