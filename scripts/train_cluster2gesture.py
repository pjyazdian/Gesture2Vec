...
from model.seq2seq_with_cluster_model import cluster2gesture_model
from train_eval.train_seq2seq import train_iter_seq2seq, train_iter_c2g_seq2seq
...
def init_model(args, lang_model, pose_dim, _device):
    ...
    generator = cluster2gesture_model(args, 40, args.hidden_size, args.hidden_size, args.rep_learning_dim, ).to(_device)
...
def train_epochs(args, train_data_loader, test_data_loader, lang_model, pose_dim, trial_id=None):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for iter_idx, data in enumerate(train_data_loader, 0):
            global_iter += 1
            in_text, text_lengths, target_vec, in_audio, aux_info, cluster_id = data
            batch_size = target_vec.size(0)

            in_text = in_text.to(device)
            # in_audio = in_audio.to(device)
            target_vec = target_vec.to(device)
            cluster_id = torch.cat(cluster_id).to(device)
            # train
            loss = train_iter_c2g_seq2seq(args, epoch, cluster_id, target_vec, generator, gen_optimizer)
...
def evaluate_testset(test_data_loader, generator, loss_fn, args):
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            # in_text, text_lengths, target_vec, in_audio, aux_info, cluster_di = data
            in_text, text_lengths, target_vec, audio, aux_info, cluster_id = data

            batch_size = target_vec.size(0)

            in_text = in_text.to(device)
            target = target_vec.to(device)
            # Todo: why the collate doesn't work for cluster_id?
            cluster_id = torch.cat(cluster_id).to(device)
            out_poses = generator(cluster_id, target)
...
def main(config):
    ...
    # dataset
    train_dataset = TrinityDataset_with_cluster(args, args.train_data_path[0],
    ...
    val_dataset = TrinityDataset_with_cluster(args, args.val_data_path[0],
                                 n_poses=args.n_poses,
                                 subdivision_stride=args.subdivision_stride,
                                 pose_resampling_fps=args.motion_resampling_framerate,
                                 data_mean=args.data_mean, data_std=args.data_std)
