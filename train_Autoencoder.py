...
from model.Autoencoder_model import Autoencoder_seq2seq
from train_eval.train_seq2seq import train_iter_seq2seq, train_iter_Autoencoder_seq2seq, train_iter_Autoencoder_ssl_seq2seq
...
def init_model(args, lang_model, pose_dim, _device):
    ...
    generator = Autoencoder_seq2seq(args, pose_dim, n_frames).to(_device)
...
def train_epochs(args, train_data_loader, train_sim_dataset, test_data_loader, lang_model, pose_dim, trial_id=None):
    ...
    if args.autoencoder_freeze_encoder == 'True':
        rnn = utils.train_utils.load_checkpoint_and_model(
            args.autoencoder_checkpoint, device, 'autoencoder')
        args_rnn, rnn, loss_fn, lang_model, out_dim = rnn
        generator = rnn
        generator.freez_encoder()
        generator.autoencoder_fixed_weight = False
        generator.autoencoder_conditioned = True
        generator.decoder.decoder.autoencoder_conditioned = True
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for iter_idx, data in enumerate(train_data_loader, 0):
            ...
            if train_labeled:
                ...
                loss = train_iter_Autoencoder_seq2seq(args, epoch,
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            ...
            if args.autoencoder_vae== 'True':
                ...
                # print(in_poses.shape,target_poses.shape)
                # print(generator)
...
def main(config):
    ...
    # # Ablation study:
    # # for autoencoder_denoising in [ 'False', 'True']:
    # counter = 0
    # ablation_folder = args.model_save_path
    # for autoencoder_att in [ 'False', 'True']:
    #     args.autoencoder_att = autoencoder_att
    #     for autoencoder_fixed_weight in [ 'False', 'True']:
    #         args.autoencoder_fixed_weight = autoencoder_fixed_weight
    #         for autoencoder_conditioned in [ 'False', 'True']:
    #             args.autoencoder_conditioned = autoencoder_conditioned
    #             for autoencoder_vae in [ 'False', 'True']:
    #                 args.autoencoder_vae = autoencoder_vae
    #                 for autoencoder_vq in [ 'False']: #, 'True']:
    #                     args.autoencoder_vq = autoencoder_vq
    #                     for use_derivitivein in [ 'False', 'True']:
    #                         args.use_derivitivein = use_derivitivein
    #                         for dropout_prob in [0.0, 0.2, 0.8]:
    #                             args.dropout_prob = dropout_prob
    #
    #                             counter = counter + 1
    #
    #                             if args.use_derivitive == 'True':
    #                                 args.rep_learning_dim *= 2
    #
    #                             args.model_save_path = ablation_folder + '/' + str(counter) + '/'
    #                             if not os.path.exists(args.model_save_path):
    #                                 os.mkdir(args.model_save_path)
    #
    #                             conf = open(args.model_save_path + 'conf', 'w')
    #                             str_conf = 'autoencoder_att = {}\n' \
    #                                   'autoencoder_fixed_weight = {}\n' \
    #                                   'autoencoder_conditioned = {}\n' \
    #                                   'autoencoder_vae = {}\n' \
    #                                   'autoencoder_vq = {}\n' \
    #                                   'use_derivitivein = {}\n' \
    #                                   'dropout_prob = {}\n'.format\
    ...
    #                             conf.flush()
    #                             conf.close()
                                # train_epochs(args, train_loader, train_dataset,
                                #              test_loader, None,
...
'''
    ...
    # for autoencoder_denoising in [ 'False', 'True']:
    counter = 0
    ablation_folder = _args.model_save_path
    for autoencoder_att in ['False', 'True']:
        _args.autoencoder_att = autoencoder_att
        for autoencoder_fixed_weight in ['False', 'True']:
            _args.autoencoder_fixed_weight = autoencoder_fixed_weight
            for autoencoder_conditioned in ['False', 'True']:
                _args.autoencoder_conditioned = autoencoder_conditioned
                for autoencoder_vae in ['False', 'True']:
                    _args.autoencoder_vae = autoencoder_vae
                    for autoencoder_vq in ['False']:  # , 'True']:
                        _args.autoencoder_vq = autoencoder_vq
                        for use_derivitivein in ['False', 'True']:
                            _args.use_derivitivein = use_derivitivein
                            for dropout_prob in [0.0, 0.2, 0.8]:
                                _args.dropout_prob = dropout_prob
                                ...
                                _args.model_save_path = ablation_folder + '/' + str(counter) + '/'
                                if not os.path.exists(_args.model_save_path):
                                    os.mkdir(_args.model_save_path)
